...
import { OrbitControls, Stars , Text, Html, useTexture } from '@react-three/drei';
...
const ParticleBackground = () => {
  ...
  return <div id="particles-js" className="fixed inset-0 z-0 pointer-events-none"></div>;
};
      ...
      <nav className="container mx-auto px-4 sm:px-6 lg:px-8 py-3">
        <div className="flex items-center justify-between">
          ...
          <div className="hidden md:flex space-x-4">
            {['Home', 'About', 'Menu', 'Gallery', 'Order', 'Locate'].map((item) => (
              <Link key={item} to={`/${item.toLowerCase()}`} className="text-white hover:text-yellow-500 transition">
                {item}
              </Link>
            ))}
          </div>
          ...
          >
            ...
            <div className="space-y-4">
              {timelineEvents.map((event) => (
                ...
                >
                  <div className="w-16 h-16 bg-yellow-500 rounded-full flex items-center justify-center mr-4 text-2xl">
                    {event.icon}
                  </div>
                  <div>
...
// Spice Explorer Component
// function SpiceExplorer() {
//   const [activeSpice, setActiveSpice] = useState(null);
//   const spices = [
//     { name: 'Turmeric', color: '#FFC300', benefits: 'Anti-inflammatory, antioxidant' },
//     { name: 'Cumin', color: '#8B4513', benefits: 'Aids digestion, rich in iron' },
//     { name: 'Coriander', color: '#228B22', benefits: 'Lowers cholesterol, rich in vitamins' },
//     { name: 'Cardamom', color: '#90EE90', benefits: 'Aids digestion, freshens breath' },
//     { name: 'Cinnamon', color: '#D2691E', benefits: 'Lowers blood sugar, antioxidant' },
//   ];

//   return (
//     <section className="py-20 bg-gray-100">
//       <div className="container mx-auto px-6">
//         <h2 className="text-4xl font-bold mb-8 text-center text-gray-800">Explore Our Spices</h2>
...
//             >
//               <h3 className="text-2xl font-semibold mb-2">{activeSpice.name}</h3>
//               <p className="text-gray-600">{activeSpice.benefits}</p>
//             </motion.div>
//           )}
//         </AnimatePresence>
//       </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div>
            ...
            <ul className="space-y-2">
              {['Home', 'About Us', 'Our Menu', 'Online Order'].map((item) => (
                <li key={item}>
                  <Link to={`/${item.toLowerCase().replace(' ', '-')}`} className="text-gray-400 hover:text-yellow-500 transition">{item}</Link>
                </li>
              ))}
            </ul>
          </div>
          <div>
            ...
            <p className="text-gray-400">123 Foodie Street, Flavortown</p>
...
function InteractiveGallery() {
  ...
  const categories = ['Indoor', 'Outdoor', 'Restaurant design'];
      ...
      <AnimatePresence>
        {selectedImage && (
          ...
          >
            ...
            <button onClick={handlePrev} className="absolute left-4 text-white text-2xl bg-black bg-opacity-50 p-2 rounded-full">&lt;</button>
            <button onClick={handleNext} className="absolute right-4 text-white text-2xl bg-black bg-opacity-50 p-2 rounded-full">&gt;</button>
            <button onClick={() => setSelectedImage(null)} className="absolute top-4 right-4 text-white text-2xl bg-black bg-opacity-50 p-2 rounded-full">×</button>
          </motion.div>
        )}
      </AnimatePresence>
    </section>
...
function NotFound() {
  return (
    <div className="flex flex-col items-center justify-center min-h-screen">
      ...
      <Link to="/" className="bg-yellow-500 text-white px-6 py-2 rounded-full hover:bg-yellow-600 transition duration-300">
        Go Back Home
      </Link>
    </div>
  );
